...
class CheckoutTest extends CatsEffectSuite {
  ...
  test("process with error cart ") {
    ...
    out.attempt.map(_.fold(t => assertEquals("error abc", t.getMessage), _ => fail("")))
  }
  ...
  test("process with non empty cart ") {
    ...
    when(orders.create(userId, pid, NonEmptyList.fromList(items).get, total))
      .thenReturn(IO(orderId))
    // act
    val out: IO[OrderId] = checkout.process(userId, card)
    // assert
    out.attempt.map(
